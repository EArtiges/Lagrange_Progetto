...
u'\u043d\u0430',u'\u043d\u0435',u'\u0442\u0443\u0440\u0438\u043d',u'\u0447\u0442\u043e','beautiful','smile','torino\xc3\xa8lamiacitt']))
...
def N_posts(df_new, debug=False):
    ...
    list_dates=[l[0:7] for l in list_dates]
...
def time_coord(df_location_all, df_new):
    ...
    """
    IDCOORD= dict(zip( df_location_all.id,df_location_all.coord))
    TID = dict(zip(df_new.location_id,df_new.date))
    
    for k in IDCOORD.keys():
...
def N_cells_info(df,step_m):
    ...
    lis_coords_Nposts=[]
    for month in dic_month_Ncells.keys():
...
def text_processing(df_new):
    ...
    df_new['text_new']=df_new['text_new'].map(lambda x: string.join([a for a in x.split(' ') if a!=''], ' '))
    #Remove punctuation
    df_new.text_new = [filter(lambda x: x not in string.punctuation,post) for post in df_new.text_new]
...
def file_tree2(seeded,expanded,n_features,sampling,flags,start,n_topics,monuments,n_posts,seed='noseed'):
    run_id=string.join([str(datetime.now().date()),str(n_topics),str(n_features)],'_')
    if monuments:
        run_id+='_noMon'
    if sampling:
        run_id+='_spl'
...
def language_counter(all_posts):
    ...
    for a in set(all_posts):
        ...
        if L in languages_counter.keys():
